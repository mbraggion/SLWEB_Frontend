...
const Contas = (props) => {
	...
	const handleOpenContaDialog = async (DOC) => {
		...
		try {
			const response = await api.get(
				`/compras/pedidos/detalhes/${DOC.E1_NUM}/DOC`
			);
			setPedidoDet({
	...
	const handleRetriveBoleto = async (Pedido) => {
		...
		try {
			...
			const response = await api.get(
				`/compras/retriveboleto/${Pedido.PedidoId}/${
					Pedido.Parcela.trim() === '' ? 'UNICA' : Pedido.Parcela.trim()
				}`,
				{
			...
			if (response.data.byteLength > 28) {
				...
				saveAs(
					blob,
					`BOLETO_NF_${Pedido.PedidoId}_${
						Pedido.Parcela.trim() === '' ? 'ÚNICA' : Pedido.Parcela.trim()
					}.pdf`
				);
				setWait(false);
			} else {
	...
	return !loaded ? (
		...
		>
			<ContaModal
				...
				onRequestNFe={(DOC) => handleRetriveNota(DOC)}
				type={tipoDuplicata}
			/>
			<PagamentoModal
					...
					<TableBody>
						{TotalAno.map((ta) => (
							<StyledTableRowWithoutHighlight>
								...
								<StyledTableCell>{currencyFormat(ta['1'])}</StyledTableCell>
								<StyledTableCell>{currencyFormat(ta['2'])}</StyledTableCell>
								<StyledTableCell>{currencyFormat(ta['3'])}</StyledTableCell>
								<StyledTableCell>{currencyFormat(ta['4'])}</StyledTableCell>
								<StyledTableCell>{currencyFormat(ta['5'])}</StyledTableCell>
								<StyledTableCell>{currencyFormat(ta['6'])}</StyledTableCell>
								<StyledTableCell>{currencyFormat(ta['7'])}</StyledTableCell>
								<StyledTableCell>{currencyFormat(ta['8'])}</StyledTableCell>
								<StyledTableCell>{currencyFormat(ta['9'])}</StyledTableCell>
								<StyledTableCell>{currencyFormat(ta['10'])}</StyledTableCell>
								<StyledTableCell>{currencyFormat(ta['11'])}</StyledTableCell>
								<StyledTableCell>{currencyFormat(ta['12'])}</StyledTableCell>
							</StyledTableRowWithoutHighlight>
						))}
					</TableBody>
				</Table>
									...
									<TableBody>
										{TotalDuplicatas.map((total) => (
											<StyledTableRowWithoutHighlight>
												...
												<StyledTableCell
													style={{
														background: total.Vencido > 0 ? '#ff4747' : null,
													}}
												>
