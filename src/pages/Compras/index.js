...
function Compras(props) {
	...
	const { LoadInsumos, DestroyStore, UpdateCarrinho, LoadMultiplicador } =
		props;

	const { TabIndex, Carrinho, Checked, Produtos, Multiplicador_Desconto } =
		props.State;
			...
			{Multiplicador_Desconto < 1 && showDescontoCard ? (
				...
				>
					...
					<Typography style={{ color: '#000' }} variant='subtitle2'>
						Existe um desconto de{' '}
						<strong>{100 - Multiplicador_Desconto * 100}%</strong> ativo para as
						suas compras de insumos, aproveite!
					</Typography>
				</div>
			) : null}
