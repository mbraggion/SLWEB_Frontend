...
const CarrinhoModal = ({ open, onClose, desconto, ...props }) => {
	...
	const handleBuy = async (event) => {
		...
		try {
			...
			Toast(
				'Não foi possível efeturar a compra. Verifique as pendencias na tela "Contas à Pagar"',
				'update',
				toastId,
				'error'
			);
	...
	return (
		<Dialog open={open} fullScreen onClose={() => onClose(false)}>
					...
					<div className='XAlign' style={{ width: 'unset' }}>
						...
						<div className='YAlign' style={{ flex: 'unset' }}>
							...
							<Typography gutterBottom variant='subtitle1'>
								...
								{totalPedido(Carrinho, aVista, desconto)}
							</Typography>
						</div>
					</div>
				</div>
...
const verifyPedido = (pedido, retira, MinCompra, aVista, Desconto) => {
	...
	if (
		!retira &&
		Number.parseFloat(totalPedido(pedido.Items, aVista, Desconto)) <
			Number.parseFloat(MinCompra)
	) {
	...
	return [
		...
		},
		{
			...
			valueGetter: (params) =>
				...
				((aVista ? 0.95 : 1) +
					(Desconto && params.getValue(params.id, 'Royalties') === 1
						? Desconto
						: 1) -
					1),
		},
