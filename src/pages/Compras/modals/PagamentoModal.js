...
function ContasModal({ open, onClose, duplicatas, confiavel, naoCompensavel }) {
	...
	const handleSubmit = async () => {
		...
		if (qtdArquivos < 1) {
			Toast(
				'Anexe pelo menos um arquivo de imagem ou PDF que comprove o pagamento da duplicata',
				'warn'
			);
			setFetching(false);
			return;
			...
			<DialogContent>
				<div className='YAlign'>
					<Typography variant='body1' gutterBottom>
						Caso o pagamento de alguma duplicata esteja levando muito tempo para
						ser confirmado pelo sistema, nos envie o comprovante de pagamento.
					</Typography>
					<div
						className='XAlign'
						style={{
										...
										<FormControlLabel
											...
											control={
												<Checkbox
													...
													checked={
														...
														confirmDuplicatas.E1_PREFIXO[0] ===
															duplicata.E1_PREFIXO[0] &&
														confirmDuplicatas.E1_PARCELA ===
															duplicata.E1_PARCELA
													}
													onChange={(e) =>
														handleClickDuplicata(e.target.checked, index)
													}
											...
											label={
												<>
													...
													{naoCompensavel.filter(
														(nc) =>
															nc.E1_PREFIXO.trim() ===
																duplicata.E1_PREFIXO[0].trim() &&
															nc.E1_NUM.trim() === duplicata.E1_NUM.trim() &&
															nc.E1_PARCELA.trim() ===
																duplicata.E1_PARCELA.trim()
													).length > 0
														? ` (em análise)`
