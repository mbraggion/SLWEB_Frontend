...
export const MoveModal = ({ open, onClose }) => {
	...
	const handleSubmit = async () => {
		...
		const res = await onMove(
			formatedFolderPath,
			String(thisPath).toString().replace(/,/g, '\\')
		);
		...
		>
			...
			>
				{tgFolder.length > 0 ? (
						...
						{tgFolder.map((f) => (
							<ListItem
								...
								disabled={
									...
									String(f.path).includes(formatedFolderPath)
								}
							>
								<ListItemAvatar>
									<Avatar>
					...
					<div
						className='YAlign'
						style={{ alignItems: 'center', margin: '0px 0px 8px 0px' }}
					>
						<FolderOpenIcon fontSize='large' />
						<Typography variant='caption'>Não há subpastas aqui.</Typography>
					</div>
				)}
