...
import { DateRange as DateRangeIcon, SyncAlt as SyncAltIcon } from '@material-ui/icons'
import { makeStyles, withStyles, List, ListItem, Button, ListItemText, ListSubheader, ListItemSecondaryAction, Checkbox, Typography } from '@material-ui/core'
import { Toast } from '../../../components/toasty'

export const LeiturasList = ({ leituras, telemetriaId, uuid, anxid, pdvid, status }) => {
        ...
        return (
            <li className={classes.listSection}>
                <ul className={classes.ul}>
                    <ListSubheader style={{ color: '#FFF', background: 'linear-gradient(160deg,#071441 13%,#25C998 100%)', boxShadow: '0px 3px 4px 0px rgb(0 0 0 / 14%)' }}>{sectionName}</ListSubheader>
                    {leit.map((l) => (
                        <ListItem button key={l.LeituraId} style={{ cursor: 'pointer' }} onClick={() => handleSelectLeitura(l.LeituraId)}>
                            <ListItemText
                                primary={moment(l.DataLeitura).add(3, 'hours').format('DD/MM/YYYY hh:mm:ss a')}
                            ...
                            <ListItemSecondaryAction>
                                <GreenCheckbox
                                    ...
                                    checked={periodo.de === l.LeituraId || periodo.ate === l.LeituraId}
                                />
                            </ListItemSecondaryAction>
                        </ListItem>
                    ))}
    ...
    const handleRequestColeta = async () => {
        ...
        try {
            const response = await api.get(`/audit/excel/${uuid}/${anxid}/${pdvid}/${periodo.de}/${periodo.ate}`, {
    ...
    return (
        <div className={classes.container}>
            <Typography variant='h5' className={classes.telemetriaHeader}>TELEMETRIA</Typography>
            <List className={classes.root} subheader={<li />}>
                {status === 'disponivel'
                    ? (
                        <>
                            {leituras.filter(l => wasToday(l)).length > 0 ? getSection(leituras.filter(l => wasToday(l)), 'Leituras de hoje') : null}
                            {leituras.filter(l => !wasToday(l) && wasThisWeek(l)).length > 0 ? getSection(leituras.filter(l => !wasToday(l) && wasThisWeek(l)), 'Leituras dos últimos 7d') : null}
                            {leituras.filter(l => !wasToday(l) && !wasThisWeek(l)).length > 0 ? getSection(leituras.filter(l => !wasToday(l) && !wasThisWeek(l)), 'Leituras anteriores') : null}
                        </>
                    )
                    : status === 'desabilitado'
                        ? (
...
const wasToday = (leitura) => {
    ...
    const maxDtDiff = 1
    const datediff = moment().subtract(3, 'h').diff(moment(leitura.DataLeitura), 'd')
