					...
					>
						{Coletas.length === 0 ? (
							<div className='XAlign'>
								<Typography>Nenhuma coleta gravada</Typography>
							</div>
						) : (
							Coletas.filter((coleta) => {
								if (typeof selectedEquip != 'undefined') {
									if (
										String(coleta.EquiCod).trim() ===
										String(selectedEquip).trim()
									) {
										return true;
									} else {
										return false;
								...
								<>
									...
									>
										<ListItemText
											primary={coleta.Anexo}
											secondary={coleta.EquiCod}
										/>
										<div className={classes.helper}>
											<Typography variant='body2' gutterBottom>
												...
												{capitalizeMonthFirstLetter(
													moment(coleta.Ref).utc().format('MMMM YYYY')
												)}
											</Typography>
