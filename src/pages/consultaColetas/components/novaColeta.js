	...
	const handleRequestDetails = async (eqdata) => {
		...
		try {
			...
			const response = await api.get(
				`/coletas/historico/${String(eqdata.EquiCod).trim()}/${eqdata.AnxId}`
			);

			setLeiturasDisponiveis(response.data.LeiturasDisponiveis);

			setMargemLeitura({
				de: response.data.UltColeta[0]
					? response.data.UltColeta[0].UltimaColeta
					: null,
				deID: response.data.UltColeta[0]
					? response.data.LeiturasDisponiveis.filter(
							(leit) => leit.LeituraId === response.data.UltColeta[0].LeituraId
					  )[0].LeituraId
					: null,
				deCont: response.data.UltColeta[0]
					? response.data.LeiturasDisponiveis.filter(
							(leit) => leit.LeituraId === response.data.UltColeta[0].LeituraId
					  )[0].Contador
					: null,
				...
				ateID: null,
				ateCont: null,
				excluir: response.data.UltColeta[0]
					? response.data.UltColeta[0].UltimaColeta
					: null,
			});
			...
			setDetalhes({
				...
				PdvId: eqdata.PdvId,
				ConId: eqdata.ConId,
				UltimaColeta: response.data.UltColeta[0]
					? response.data.UltColeta[0].UltimaColeta
					: null,
				ProximaColeta: response.data.UltColeta[0]
					? response.data.UltColeta[0].ProximaColeta
					: null,
				ContadorAnterior: response.data.UltColeta[0]
					? response.data.UltColeta[0].ContadorAnterior
					: null,
				ProximaColetaMes: response.data.UltColeta[0]
					? response.data.UltColeta[0].ProximaColetaMes
					: null,
				Zerou: response.data.UltColeta[0]
					? response.data.UltColeta[0].Zerou
					: null,
			});
		} catch (err) {}
	};
		...
		async function getQtd() {
			try {
				if (margemLeitura.de !== null && margemLeitura.ate !== null) {
					...
					const response = await api.get(
						`/coletas/novacoleta/${margemLeitura.deID}/${margemLeitura.ateID}/${detalhes.AnxId}/${detalhes.PdvId}`
					);
	...
	return isMdUp ? (
		<Paper className={classes.root}>
			<div className={classes.container}>
				<Typography variant='h5' style={{ margin: '8px 0px 0px 8px' }}>
					Nova Coleta
				</Typography>
				<NovaColetaContent
					classes={classes}
