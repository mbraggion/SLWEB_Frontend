...
export const RaspyContainer = ({ selectedAnx, onSelectAnx, selectedAnxName }) => {
  ...
  const fetchRaspy = async () => {
    ...
    try {
      const response = await api.get(`/raspy/${selectedAnx}/${selectedMargem.de}/${selectedMargem.ate}`)

      Toast('Vendas encontradas', 'update', toastId, 'success')
      setRaspy(response.data.raspy)
      setFetching(false)
    } catch (err) {
      Toast('Falha ao recuperar vendas do servidor', 'update', toastId, 'error')
      setRaspy([])
      setSelectedMargem({ de: null, ate: null })
      setFetching(false)
    }
