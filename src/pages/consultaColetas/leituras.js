...
export const Leituras = ({ match }) => {
	...
	const [novaColetaDetalhesModalOpen, setNovaColetaDetalhesModalOpen] =
		useState(false);
	...
	const handleOpenColetaDetailsModal = async (anxid, pdvid, fseq, coleta) => {
		...
		try {
			const response = await api.get(
				`/coletas/detalhes/${anxid}/${pdvid}/${fseq}`
			);
	...
	return !loaded ? (
		...
		<div className={classes.root}>
			...
			<Consultas
				Coletas={coletas}
				onOpenColetaDetails={(a, p, f, c) =>
					handleOpenColetaDetailsModal(a, p, f, c)
				}
