	...
	const LoadData = async () => {
		try {
			const response = await api.get(
				`/contracts/info/anexo/${String(contract.CNPJ).trim()}/${
					contract.ConId
				}`
			);
		...
		<div className={classes.container}>
			...
			<div className={classes.linha} style={{ margin: '8px 0px' }}>
				<TextField
					...
					onChange={(e) =>
						setInfoAnexo({ ...infoAnexo, AnxDiaFecha: e.currentTarget.value })
					}
					disabled={allowEdit}
			...
			<div className={classes.linha}>
				<TextField
					...
					onChange={(e) =>
						setInfoAnexo({ ...infoAnexo, AnxObs: e.currentTarget.value })
					}
					disabled={allowEdit}
					label='Observações'
					multiline
