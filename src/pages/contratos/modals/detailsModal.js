...
export const DetailsModal = ({
	...
	const handleInativar = async (CNPJ, ConId, currentStatus) => {
		...
		toastId = Toast(
			currentStatus === 'A' ? 'Inativando...' : 'Reativando...',
			'wait'
		);
		setWait(true);
		...
		try {
			...
			Toast(
				currentStatus === 'A' ? 'Contrato inativado' : 'Contrato reativado',
				'update',
				toastId,
				'success'
			);
			...
			Toast(
				currentStatus === 'A'
					? 'Falha ao inativar contrato'
					: 'Falha ao reativar contrato',
				'update',
				toastId,
				'error'
			);
					...
					>
						{allowEditing ? (
							<Button
								disabled={wait}
								onClick={() =>
									handleInativar(target.CNPJ, target.ConId, target.ConStatus)
								}
								color='primary'
								startIcon={
									target?.ConStatus === 'A' ? (
