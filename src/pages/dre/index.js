...
const DRE = () => {
	...
	const ano =
		moment(selRef).add(3, 'hours').month() === 0
			? moment(selRef).year() + 1
			: moment(selRef).year();
	...
	const handleDownloadExcel = async (type) => {
		...
		try {
			if (type === 'DRE') {
				const response = await api.get(`/dre/excel/dre/${ano}/${mes}`, {
				...
				const response = await api.get(`/dre/excel/baseroy/${ano}/${mes}`, {
					responseType: 'arraybuffer',
				});
			...
			>
				<div className='XAlign' style={{ alignItems: 'flex-start' }}>
					...
					<section className={classes.metadinha}>
						<Resumo Res={dre.filter((d) => d.DreCod < 23 || d.DreCod === 35)} />
					</section>
					<section className={classes.metadinha}>
						<Despesas
