...
const Requisicao = (props) => {
	...
	const [modalInstrucaoPrivateLabel, setModalInstrucaoPrivateLabel] =
		useState(false);
		...
		if (maq === '') {
			...
			Ativos.map((ativo) => {
				if (ativo.MaqModelo === maq) {
					ChangeMaquina(ativo);
					api
						.get(`/equip/requests/default/${ativo.MaqModId}`)
						.then((response) => {
	...
	const handleChangeSistemaDePagamento = (value) => {
		if (
			value === 'Cartão' ||
			value === 'Cartão e Validador' ||
			value === 'Cartão e Private Label'
		) {
		...
		<div className='YAlign'>
			...
			>
				...
				>
					...
					<MenuItem value='Cartão e Private Label'>
						Cartão e Private Label
					</MenuItem>
				</Selecao>
				...
				>
					...
					<p style={{ margin: '0px' }}>
						{Contenedor.length}/<strong>{MaxContenedores}</strong> Contenedores
						em uso
					</p>
				</div>
