...
export const Form = ({
	...
	const sessao =
		!loading && !isFormComplete ? Form[Object.keys(Form)[section]] : [];
	const questao =
		...
		if (q.dependences !== null) {
			...
			q.dependences.forEach((d) => {
				...
				if (targetQuestion === null)
					throw new Error('questão dependida não encontrada');
	...
	const handleRequestAdvance = async () => {
		...
		if (
			...
			section + 1 === Object.keys(Form).length
		) {
			await finishForm();
		...
		let newSection =
			question + 1 === sessao.length &&
			section + 1 !== Object.keys(Form)[section + 1].length
				? section + 1
				: section;
		let newQuestion =
			question + 1 === sessao.length &&
		...
		for (
			let s = newSection;
			s < Object.entries(Form).length && !validated;
			s++
		) {
			for (
				let q = newQuestion;
				q < Form[Object.keys(Form)[s]].length && !validated;
				q++
			) {
	...
	const handleRequestRetreat = async () => {
		...
		let newQuestion =
			question === 0
				? Form[Object.keys(Form)[section - 1]].length - 1
				: question - 1;
