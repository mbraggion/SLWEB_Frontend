	...
	return loading ? (
		...
		<div className='YAlign'>
			...
			<Typography>
				...
				{franquia.CAR.Clientes.reduce((acc, t) => {
					return acc + Number(t.Qtd);
				}, 0)}{' '}
				[{franquia.CAR.Clientes.filter((t) => t.Status === 'A')[0].Qtd} Ativos]
			</Typography>
			...
			>
				<Tooltip
					...
					title={
						...
						>
							{franquia.CAR.AtivosStatus.filter(
								(AS) => AS.LeitOk.trim() === 'OK'
							).map((AS) => (
								<Typography color='inherit' style={{ whiteSpace: 'nowrap' }}>
									{AS.EquiCod} -&gt; {montaEndereco(AS)}
								</Typography>
							))}
						</div>
					}
			...
			<Typography>
				Pode emitir nota:{' '}
				<strong>{franquia.FIN.EmiteNF === 'S' ? 'Sim' : 'Não'}</strong>
			</Typography>
			...
			<Typography>
				...
				<strong>
					{new Intl.NumberFormat('pt-BR', {
						style: 'currency',
						currency: 'BRL',
					}).format(franquia.FIN.LimiteExtra)}
				</strong>{' '}
				{franquia.FIN.DtExtraConcedido !== null
					? `[válido apenas em ${moment(franquia.FIN.DtExtraConcedido).format(
							'L'
					  )}]`
					: null}
			</Typography>
			...
			<Typography>
				...
				<strong>
					{new Intl.NumberFormat('pt-BR', {
						style: 'currency',
						currency: 'BRL',
					}).format(franquia.FIN.MinCompra)}
				</strong>
			</Typography>

			<Typography>
				Confiável:{' '}
				<strong>{franquia.FIN.Confiavel === true ? 'Sim' : 'Não'}</strong>
			</Typography>

			<Typography>
				Pode retirar:{' '}
				<strong>{franquia.FIN.PodeRetirar === 'S' ? 'Sim' : 'Não'}</strong>
			</Typography>

			<Typography>
				Bloqueado:{' '}
				<strong>{franquia.FIN.Bloqueado === 'S' ? 'Sim' : 'Não'}</strong>
			</Typography>
