...
const emAndamento = ({ OS, onRequestPDF, onRefresh }) => {
	return (
		<Table width={100} height={50} hoverable={true} responsive={false} centered>
			<thead>
				<tr>
					<th>Solicitação</th>
					<th>Filial</th>
			...
			<tbody>
				{OS.map((OS) => (
					<tr>
						...
						<td align='center'>
							<strong>{OS.Stage}</strong>
						</td>
						<td align='center'>{convertData(OS.Datas.OSCDtSolicita)}</td>
						<td align='center'>{convertData(OS.Datas.OSCDtPretendida)}</td>
						<td align='center'>
							{' '}
							{OS.Assinaturas.OSCTecDtPrevisao !== ''
								? convertData(OS.Assinaturas.OSCTecDtPrevisao)
								: 'NA'}{' '}
						</td>
						<td align='center'>
							{' '}
							{OS.Assinaturas.OSCExpDtPrevisao !== ''
								? convertData(OS.Assinaturas.OSCExpDtPrevisao)
								: 'NA'}{' '}
						</td>
						<td align='center'>
							{' '}
							<AdmDialog Req={OS} onRefresh={onRefresh} />{' '}
						</td>
						<td>
							{' '}
							<HistDialog Req={OS} />{' '}
						...
						<td>
							{' '}
							<Button
								style={{ color: '#FFFFFF', backgroundColor: RED_SECONDARY }}
								onClick={() => onRequestPDF(OS.OSCId)}
							>
								{' '}
								<FindInPage />{' '}
							</Button>{' '}
