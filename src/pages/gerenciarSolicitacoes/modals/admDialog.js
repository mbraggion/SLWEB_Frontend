		...
		<div>
			...
			>
				...
				<DialogContent>
					...
					{ShowControlls(CheckStage(Req), Req, handleClose, onRefresh)}
				</DialogContent>
...
const ShowControlls = (stage, Req, handleClose, onRefresh) => {
	if (
		(stage === 999 || stage === 0) &&
		roleLevel() !== REACT_APP_SISTEMA_ROLE_LEVEL
	) {
		...
		switch (roleLevel()) {
			case REACT_APP_SISTEMA_ROLE_LEVEL:
				...
				return (
					<Sistema Req={Req} onClose={handleClose} onRefresh={onRefresh} />
				);
			case REACT_APP_FRANQUEADO_ROLE_LEVEL:
				//Franqueado

				return (
					<Franqueado Req={Req} onClose={handleClose} onRefresh={onRefresh} />
				);
