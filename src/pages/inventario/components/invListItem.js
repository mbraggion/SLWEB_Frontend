...
import { Accordion, AccordionActions, AccordionDetails, AccordionSummary, Button, Divider, ListItemText, makeStyles, TextField } from '@material-ui/core'
...
export const InvListItem = ({ InvItem, isInventoryClosed, expandedId, updateInventory, selectedDepId, selectedRef, onExpandProd }) => {
  ...
  const handleSaveAjuste = async () => {
    ...
    try {
      await api.put(`/inventario/${selectedDepId}/${encodeURI(selectedRef)}/${InvItem.ProdId}`, {
        InvAjQ: ajusteQtd,
  ...
  return (
    ...
    >
      ...
      <AccordionDetails className={classes.details}>
        <InvListMovList>
          {InvItem.Mov.map(mov => <InvListMovItem Mov={mov} />)}
        </InvListMovList>
      </AccordionDetails>
      <Divider />
      <AccordionActions>
