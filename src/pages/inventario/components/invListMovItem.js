	...
	return (
		<tr>
			...
			<td
				className={clsx(
					classes.lineDoc,
					Mov.A1_NOME === 'INVENTÁRIO INICIAL' ||
						Mov.A1_NOME === 'INVENTÁRIO FINAL'
						? classes.black
						: Mov.E !== 0
						? classes.green
						: Mov.S !== 0
			...
			<td
				className={clsx(
					classes.lineDoc,
					Number(whichSaldo(Mov.A1_NOME, Mov.SldAnt, Mov.SldAtu)) >= 0
						? classes.black
						: classes.red
				)}
			>
			...
			<td className={clsx(classes.lineDoc, classes.line)}>{Mov.A1_NOME}</td>
		</tr>
	);
};
