...
export const InventarioOptions = ({
	updateInventory,
	selectedRef,
	availableRefs,
	onUpdateRef,
	...
	return (
		<section className={classes.root}>
			<div className={classes.infoContainer}>
				...
				<div>
					{whichActions(
						Inventario ? Inventario.status : null,
						produtosZerados,
						wait,
						handleCreateNewInv,
						handleCloseInv,
			...
			<FormControl variant='outlined' className={classes.formControl}>
				...
				>
					...
					{availableRefs.map((ref) => (
						<MenuItem key={ref.RefUd} value={ref.RefUd}>
							{moment(ref.RefUd).format('MM/YYYY')}
						</MenuItem>
					))}
				</Select>
			</FormControl>
					...
					<Tooltip
						title={
							<div
								style={{ fontSize: '14px', color: '#FFF', lineHeight: '20px' }}
							>
								Descarta do inventário todos os produtos que tenham saldo 0 e
								que não tenham sido movimentados no mês
							</div>
						}
						placement='top'
						arrow={true}
