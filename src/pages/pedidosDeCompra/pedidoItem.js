		...
		<Accordion
			...
			style={{
				borderLeft: `4px solid ${
					Pedido.Status === 'Aguardando' ? '#4f9eff' : '#29ff8d'
				}`,
			}}
			onChange={() =>
				handleChangeExpandedAccordion(
			...
			<AccordionSummary expandIcon={<ExpandMoreIcon />}>
				<div className={classes.column}>
					<Typography className={classes.heading}>
						Pedido de compra <strong>{Pedido.PedidoID}</strong>
					</Typography>
					...
					<Typography className={classes.heading}>
						...
						<strong>
							{new Intl.NumberFormat('pt-BR', {
								style: 'currency',
								currency: 'BRL',
							}).format(Pedido.SomaDePrecoTotal)}
						</strong>
					</Typography>
					<Typography className={classes.secondaryHeading}>
						Items: <strong>{Pedido.ContarDePedidoItemID}</strong>
					</Typography>
				</div>
				<div className={classes.column}>
					<Typography className={classes.heading}>
					...
					<Typography className={classes.secondaryHeading}>
						Código <strong>{Pedido.CodigoCliente}</strong>
					</Typography>
				</div>
			</AccordionSummary>
			<Divider />
			<AccordionDetails className={classes.details}>
				<div className={clsx(classes.column_1, classes.helper)}>
					{Pedido.Detalhes.map((item) => (
						<div key={item.PedidoItemID} className={classes.prodLine}>
							<div>
								<Typography className={classes.heading}>
				...
				<div className={clsx(classes.column_2, classes.helper)}>
					...
					<FormControl variant='outlined' className={classes.formControl}>
						...
						>
							...
							<MenuItem value='00'>
								Não transmite nota, não gera boleto
							</MenuItem>
						</Select>
					</FormControl>
					<FormControl variant='outlined' className={classes.formControl}>
						...
						>
							...
							{Transportadoras.map((tr) => (
								<MenuItem key={tr.A4_COD} value={tr.A4_COD}>
									{tr.A4_NREDUZ}
								</MenuItem>
							))}
						</Select>
					...
					<div className={classes.align} style={{ alignItems: 'baseline' }}>
							...
							<KeyboardDatePicker
								...
								onChange={(value) => {
									if (
										...
										!value
											.startOf('day')
											.isBefore(moment().startOf('day').toDate()) &&
										value.startOf('day').isValid()
									) {
						...
						<PesoInput Peso={peso} onChangePeso={setPeso} disablde={wait} />
					</div>
					<TextField
