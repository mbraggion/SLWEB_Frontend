...
export const PedidosDeVendaListItem = ({
	pedido,
	ExpandedID,
	...
	return (
		<Accordion
			...
			onChange={() =>
				handleChangeExpandedAccordion(
					ExpandedID === pedido.PedidoID ? null : pedido.PedidoID
				)
			}
		>
			<AccordionSummary expandIcon={<ExpandMoreIcon />}>
				<div className={classes.column}>
					<Typography className={classes.heading}>
						Pedido <strong>{pedido.PedidoID}</strong>
					</Typography>
					<Typography className={classes.secondaryHeading}>
						Tipo:{' '}
						<strong>
							{String(pedido.PvTipo).trim() === 'V'
								? 'Venda'
								: String(pedido.PvTipo).trim() === 'R'
								? 'Remessa'
								: String(pedido.PvTipo).trim() === 'B'
								? 'Bonificação'
								: '???'}
						</strong>
					</Typography>
					<Typography className={classes.secondaryHeading}>
						Status: <strong>{returnStatusDescription(pedido)}</strong>
					</Typography>
				</div>
				<div className={classes.column}>
					<Typography className={classes.heading}>
						Filial: <strong>{pedido.Filial}</strong>
					</Typography>
					<Typography className={classes.secondaryHeading}>
						Franqueado: <strong>{pedido.GrupoVenda}</strong>
					</Typography>
					<Typography className={classes.secondaryHeading}>
						Solicitado:{' '}
						<strong>{moment(pedido.DataCriacao).format('L')}</strong>
					</Typography>
				</div>
				<div className={classes.column}>
					<Typography className={classes.heading}>
					...
					<Typography className={classes.secondaryHeading}>
						Código{' '}
						<strong>
							{pedido.CodigoCliente}[{String(pedido.LojaCliente).trim()}]
						</strong>
					</Typography>
				</div>
			</AccordionSummary>
			...
			<AccordionDetails className={classes.details}>
				<div className={clsx(classes.column_1, classes.helper)}>
					{pedido.Itens.map((item) => (
						<div key={item.PedidoID} className={classes.prodLine}>
							<div>
								<Typography className={classes.heading}>
									<strong>{String(item.Produto).split('(')[0]}</strong>
								</Typography>
								<Typography variant='caption'>
									(Cód <strong>{item.CodigoProduto}</strong>)
								</Typography>
							</div>
							<div>
								<Typography className={classes.heading}>
									Qtd: <strong>{item.QtdeVendida}</strong>
								</Typography>
								<Typography variant='caption'>
									(Desc. Un.
									<strong>
								...
								<Typography variant='caption'>
									...
									<strong>
										{new Intl.NumberFormat('pt-BR', {
											style: 'currency',
											currency: 'BRL',
										}).format(item.PrecoUnitarioLiquido)}
									</strong>
									)
								</Typography>
							</div>
							...
							<ul style={{ padding: '0 0 0 8px' }}>
								{pedido.logDoPedidoNoNasajon.length > 0 ? (
									pedido.logDoPedidoNoNasajon.map((log) => (
										<li style={{ listStyleType: 'disc' }}>{log.mensagem}</li>
									))
								) : (
...
const returnBorderColor = (pedido) => {
	if (
		pedido.emitenteNoNasajon === false ||
		pedido.destinatarioNoNasajon === false
	) {
