...
export const Configuracao = forwardRef(
	({ PdvId, AnxId, allowEditing }, ref) => {
		const [configPDV, setConfigPDV] = useState([]);
		const [configPadrao, setConfigPadrao] = useState([]);
		const [configPadraoSelecionada, setConfigPadraoSelecionada] =
			useState(null);
		...
		async function LoadData() {
			const response = await api.get(
				`/pontosdevenda/info/${PdvId}/${AnxId}/config`
			);
		...
		const handleAddConfig = () => {
			setConfigPDV((oldState) => {
				...
				let nextSel =
					arrayOrganizadoPorSel.length === 0
						? 1
						: arrayOrganizadoPorSel[arrayOrganizadoPorSel.length - 1].Sel + 1;
		...
		const handleLoadConfig = () => {
			const targetConfigToLoad = configPadrao.filter(
				(cfg) => cfg.CfgId === configPadraoSelecionada
			)[0];

			setConfigPDV(
				targetConfigToLoad.Produtos.map((hm) => ({
					Sel: hm.Sel,
					ProdId: hm.ProdId,
