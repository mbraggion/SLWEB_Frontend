		...
		>
			...
			>
				...
				<DialogContent>
					<div
						className='XAlign'
						style={{
							justifyContent: FixPedido !== null ? 'space-between' : 'flex-end',
							flexWrap: 'wrap',
						}}
					>
						{FixPedido && (
							...
							>
								<strong style={{ color: '#000' }}>Atualizando pedido:</strong>{' '}
								{FixPedido}
							</Typography>
						)}
			...
			>
				<Zoom
					...
					style={{
						transitionDelay: `${
							TabIndex === 0 ? transitionDuration.exit : 0
						}ms`,
						zIndex: '2',
					}}
					unmountOnExit
...
const totalPedido = (carrinho) => {
	...
	carrinho.forEach((item) => {
		aux +=
			item.FatConversao !== null
				? (item.VVenda - item.DVenda) * (item.QVenda * item.FatConversao)
				: (item.VVenda - item.DVenda) * item.QVenda;
	});
				...
				Number.parseFloat(
					params.getValue(params.id, 'Quantidade') *
						...
						(params.getValue(params.id, 'Vlr') -
							params.getValue(params.id, 'Desconto'))
				).toFixed(2)
			).replace('.', ','),
	},
];
...
const verifyDTO = (Pedido) => {
	...
	if (
		Pedido.TipoVenda === 'R' &&
		(Pedido.RemOrigem === '' || Pedido.RemDestino === '')
	) {
...
const inputNumberString2Number = (numericString) => {
	return Number(String(numericString).replace('.', '').replace(',', '.'));
};
